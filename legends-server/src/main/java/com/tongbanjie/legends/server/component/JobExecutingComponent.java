...
public class JobExecutingComponent {
	...
	private Logger logger = LoggerFactory.getLogger(JobExecutingComponent.class);
	 ...
	 * 处理正在执行的 JOB
	 * @param jobSnapshotId
	 * @return 返回 true 视为任务执行中，会再次获取结果；false 则认为已经结束，不再处理
	 */
	...
	public boolean handleExecuting(Long jobSnapshotId) {
		...
		try {
			...
			if (jobSnapshotId == null) {
				logger.warn("获取任务结果时，jobSnapshotId 为 null");
				return false;
			}
			...
			if (jobSnapshot == null) {
				// 清理历史任务时，可能造成此情况。
				logger.warn("获取任务结果时，数据库中找不到JobSnapshot, id:{}",jobSnapshotId);
				return false;
			}
